...
2. examplePost: 画像に書かれているテキスト本文を抽出（OCR）してください。もし画像内に「複数の連なる投稿（スレッド・リプライ形式）」が含まれている場合、それぞれのリプライ（投稿）の間に必ず |||THREAD||| という区切り文字を挿入して結合してください。（UIの文字や関係ないボタンのテキストは排除し、投稿者の書いた純粋なテキストのみを抽出してください。）

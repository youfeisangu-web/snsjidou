...
`
          ...
          ? '【文字数の指定（超重要）】\n全体の約30%〜50%は「たった1〜2行だけの極めて短い一言投稿（例：一言だけの鋭いツッコミや格言）」にして、残りはスレッド等の長文にするなど、長さに極端なメリハリをつけてください。'
            : '【文字数の指定】\n毎回長文だと読者が疲れます。全体の約30%は「たった1〜2行だけの極めて短い一言投稿」を**必ず**交ぜて、長文と短文をバランス良く出力してください。';

          // 最近の投稿履歴を取得してプロンプトに混ぜる（重複防止）
          const recentHistory = await prisma.post.findMany({
            where: { profileId: profile.id, content: { not: '' } },
            orderBy: { id: 'desc' },
            take: 20,
            select: { content: true }
          });
          ...
          const historyContext = recentHistory.length > 0 
            ? `\n【過去の投稿履歴（※話題・事例の重複厳禁）】\n以下はあなたがこれまでに作成した最近の投稿履歴です。\nこれらと「同じ話題」「同じ会社や事例」「同じ結論・ニュアンス」は **絶対に** 避けてください。時間が経っていても同じ話題をこすらないでください。常に新しい、これまで触れていない独自の切り口や別の事例を用いてください。\n\n${recentHistory.map((h, idx) => `・過去投稿${idx + 1}: ${h.content.substring(0, 100)}...`).join('\n')}\n`
            : '';
...
【品質と長さのコントロール（重要）】
出力は必ずJSON形式の配列で返してください。文字数が上限に達して途中で文章が切れ、JSONエラーになることを絶対に避けてください。
文字数が長くなりすぎる場合は、生成する投稿の数を減らしてでも、必ず「完全なJSON配列」として最後まで確実に出力しきってください。
また、無意味で低品質な文章（ご自身のプロンプト指示の復唱など）は避けて、ユーザーにとって価値のある魅力的でクオリティの高い内容のみを生成してください。
